...
namespace OS.Http.Connect
{
    public abstract class HttpBase
    {
        ...
        /// </summary>
        /// <returns></returns>
        protected virtual HttpWebRequest ConfigureWebRequest(OsHttpRequest request)
        {
            Uri uri = BuildUri(request);   //  创建url请求
            var webRequest = (HttpWebRequest)WebRequest.Create(uri);

            ServicePointManager.Expect100Continue = false;
            if (request.IsDecompres)
            {
        ...
        protected virtual Uri BuildUri(OsHttpRequest request)
        {
        ...
        protected virtual void PrepareHeaders(HttpWebRequest webRequest, OsHttpRequest request)
        {
        ...
        /// <param name="webRequest"></param>
        /// <param name="request"></param>
        protected virtual void PrepareCookies(HttpWebRequest webRequest, OsHttpRequest request)
        {
        ...
        protected virtual void SendBodyDataSync(HttpWebRequest webRequest, OsHttpRequest request)
        {
            ...
            Action<OsHttpResponse> action)
        {
            if (request.HttpMothed == HttpMothed.POST && request.FormParameters.Count > 0 || request.HasFile)
            {
                ...
                webRequest.BeginGetRequestStream(ar => RquestStreamCallBack(ar, request, action), webRequest);
            }
            else
            {
                webRequest.BeginGetResponse(ar => ResponseCallBackAsync(ar, action), webRequest);
            }
        }
        ...
        private void RquestStreamCallBack(IAsyncResult result, OsHttpRequest request,
            Action<OsHttpResponse> action)
        {
        ...
        /// <param name="result"></param>
        /// <param name="action"></param>
        private void ResponseCallBackAsync(IAsyncResult result, Action<OsHttpResponse> action)
        {
            var response = new OsHttpResponse();
        ...
        /// <param name="request"></param>
        protected void WriteMultipartFormData(Stream webRequestStream, OsHttpRequest request, string boundary)
        {
        ...
        /// <param name="request"></param>
        /// <returns></returns>
        protected static string EncodeFormParameters(OsHttpRequest request)
        {
        ...
        /// <returns></returns>
        protected virtual void GetResponse(HttpWebRequest request, ref OsHttpResponse response)
        {
        ...
        private static void ExtractResponseData(OsHttpResponse response, HttpWebResponse webResponse)
        {
            using (webResponse)
            {
